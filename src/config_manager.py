...
class SSHConfigManager:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        self.config_dir = config_dir or Path.home() / ".tengingarstjóri"
        self.ssh_dir = Path.home() / ".ssh"
        self.main_ssh_config = self.ssh_dir / "config"
        self.managed_config = self.ssh_dir / "config.tengingarstjóri"
    ...
    def _update_ssh_config(self):
        ...
        try:
            ...
            with open(self.managed_config, 'w') as f:
                f.write("# Tengingarstjóri SSH Connections\n")
    ...
    def discover_ssh_keys(self) -> List[str]:
        ...
        for key_file in self.ssh_dir.glob("*"):
            if key_file.is_file() and not key_file.suffix and key_file.name not in key_patterns:
